...
def show_top_predictions():
    ...
    print(df.sort_values("final_score", ascending=False)[[ 
        "player_name", "country", "datagolf_rank", "final_score", "your_fair_odds", "bet365_odds", "win"
    ]].head(25).to_string(index=False))

def show_top_value_bets():
    df = pd.read_csv("data/ranked_predictions_with_form.csv")
    df = df[~((df["augusta_fit_score"] == 0) & (df["sg_composite_fit_score"] == 0))]
    print("\n💰 Top 25 Value Bets (With Model Input):\n")
    print(df.sort_values("expected_value", ascending=False)[[
        "player_name", "country", "datagolf_rank", "expected_value", "bet365_odds", "your_fair_odds", "roi_percent"
    ]].head(25).to_string(index=False))
...
def show_live_value_bets():
    ...
    print(df.sort_values("live_expected_value", ascending=False)[[
        "player_name", "country", "datagolf_rank", "round", "thru", "today",
...
def inspect_player():
    ...
    for _, row in matches.iterrows():
        ...
        print(f"🧠  DataGolf Rank:           {row.get('datagolf_rank', 'N/A')}")
